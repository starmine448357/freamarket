  ...
  <form class="form form--narrow"
    ...
    <div class="section">
      ...
      <div class="field">
        ...
        <input class="input" id="title" type="text" name="title" value="{{ old('title', $item->title) }}">
        @error('title')<div class="error">{{ $message }}</div>@enderror
      </div>

      <div class="field">
        <label class="label" for="brand">ブランド名</label>
        <input class="input" id="brand" type="text" name="brand" value="{{ old('brand', $item->brand) }}">
        @error('brand')<div class="error">{{ $message }}</div>@enderror
      </div>

      <div class="field">
        <label class="label" for="description">商品の説明</label>
        <textarea class="textarea" id="description" name="description" rows="5">{{ old('description', $item->description) }}</textarea>
      ...
      <div class="row">
        <div class="field">
          ...
          <input class="input" id="price" type="number" name="price" value="{{ old('price', $item->price) }}">
          @error('price')<div class="error">{{ $message }}</div>@enderror
        </div>
        <div class="field">
          <label class="label" for="condition">状態</label>
        ...
        @foreach($categories as $cat)
          <label class="chip chip--select">
            <input class="checkbox"
                   ...
                   name="categories[]"
                   value="{{ $cat->id }}"
                   @checked($oldCats->contains($cat->id))>
